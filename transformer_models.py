...
def _prepare_rgb_and_fft_tensors(
    ...
    Takes a PIL image and returns:
      - rgb_tensor: (1, 3, H, W) - normalized with ImageNet stats for ViT
      - fft_tensor: (1, 1, H, W)
    """
    from torchvision import transforms
    
    # RGB: Use ViT normalization (ImageNet stats)
    ...
    rgb_tensor = rgb_transform(pil_rgb).unsqueeze(0).to(device)  # (1, 3, H, W)

    # FFT: Compute and normalize
    fft_np = _compute_fft_image(pil_img, size=size)  # (H, W) in [0,1]
...
def swin_video_pred(
    ...
    @lru_cache(maxsize=2)
    def _get_video_swin_model(device_str, num_frames):
        model = VideoSwinRGBFFT(num_frames=num_frames, pretrained=True).to(torch.device(device_str))
        model.eval()
        return model
    
    device_str = str(device)
    model = _get_video_swin_model(device_str, T)
